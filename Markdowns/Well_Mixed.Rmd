...
```{r}
singleDayWellMixed <- function(roosts, roost_parms, inf_parms, model) {
  new_roosts <- well_mixed_movement(roosts, num_roosts=roost_parms[1])
  new_roosts <- daytime(new_roosts, roost_parms, SIRS_parms, model)
  return(new_roosts)
}
```
...
for(i in 2:500) {
  roost_series[[i]] = singleDayWellMixed(roost_series[[i-1]], roost_parms, SIRS_parms, model="SIRS")
}
